...
async function appendPermissions() {
  rbacRulesStore.permissions.forEach(permission => {
    permissionsById.value.permissions.forEach(pById => {
      if (permission.id === pById.id)
...
async function changePermissions() {
  ...
  const result = await rbacRulesStore.changePermissions(append, rolePermissionId.value)
  if (result)
